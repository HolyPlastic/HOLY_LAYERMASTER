...
- All colours in `:root` as `var(--hlm-*)` tokens. No inline hex/rgb/hsl in classes.
- Variable names describe **slot**, not value: `--hlm-primary`, not `--hlm-pink`.
- Exception: monochrome values may be named descriptively (`--hlm-void`, `--hlm-white`).
- Three-variant pattern for every interactive element:
  ```css
  --hlm-[slot]-base     /* primary state */
  --hlm-[slot]-dark     /* dark fill */
  --hlm-[slot]-boost    /* hover/active */
  ```
