...
- 5: Upgraded `_initSectionDrag()` in `js/hlm-dragdrop.js` to eagerly tag `.section-header` with `draggable="true"` while explicitly tagging `.section-star` with `draggable="false"`. Added drag target exclusion for the star button to ensure expand/collapse clicks pass through cleanly without triggering panel dragging.